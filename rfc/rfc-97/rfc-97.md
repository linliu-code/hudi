...
| ------------------------------------------- | ------------------------------------------ |------------------------------------------------------------------------|----------------------------------------------------| ---------------------------------------------------------------------------------------------------------------- |
| `OverwriteWithLatestAvroPayload`            | `COMMIT_TIME_ORDERING`                     | Set merge mode and remove payload class config                         | None                                               | Most common for bulk ingest. Removing payload makes delete marker support consistent across COW/MOR.             |
| `DefaultHoodieRecordPayload`                | `EVENT_TIME_ORDERING`                      | Set `hoodie.write.enable.event.time.watermark.in.commit.metadata=true` | No action                                          | Default since Hudi 0.5.0; behavior unchanged.                                                                    |
| `EventTimeAvroPayload`                      | `EVENT_TIME_ORDERING`                      | Set merge mode and remove payload class config                         | None                                               | Needed for out-of-order ingestion from Kafka, Pulsar, Flink. Deprecated in favor of `EVENT_TIME_ORDERING`.       |
| `FirstValueAvroPayload`                     | N/A                                        | Stop support unless explicit merger class is defined                   | Users should define merger logic if needed         | Rarely used in OSS. If multiple rows have same ordering value, older wins. Use case: dedup based on first-write. |
| `OverwriteNonDefaultsWithLatestAvroPayload` | `COMMIT_TIME_ORDERING` + `IGNORE_DEFAULTS` | Use `IGNORE_DEFAULTS` instead                                          | Ensure default logic is consistent across versions | Solves issues like empty strings treated as valid values. Cleaner than previous manual null checks.              |
| `PartialUpdateAvroPayload`                  | `EVENT_TIME_ORDERING` + `KEEP_VALUES`      | Replace with partial update config                                     | Replace payload config                             | Common in streaming UPSERT. Used in streaming CDC pipelines.                                                     |
| `AWSDmsAvroPayload`                         | `COMMIT_TIME_ORDERING`                     | Table property to signal delete markers, merger handles logic          | Readers need patch first                           | Fixes delete handling in MoR read paths; used in AWS DMS-based ingestion.                                        |
| `MySqlDebeziumAvroPayload`                  | `EVENT_TIME_ORDERING`                      | Introduce `LegacyMySqlDebeziumAvroMerger`; payload deprecated          | No migration needed for old tables                 | Required for `_event_seq` numeric comparison logic. Important in banking/transactional ingestion.                |
| `PostgresDebeziumAvroPayload`               | `EVENT_TIME_ORDERING` + `IGNORE_MARKERS`   | Use marker value (`__debezium_unavailable_value`)                      | Must rewrite data for consistency                  | CDC systems like Debezium mark unavailable fields. Full compaction is needed to migrate.                         |
| `ExpressionPayload`                         | N/A                                        | Leave unchanged                                                        | None                                               | Used in `update(...) where` logic in SQL. Will eventually be rewritten into a merger.                            |
| `HoodieMetadataPayload`                     | N/A                                        | Retain merger-style abstraction                                        | None                                               | Not impacted. Handles metadata table compactions. Merges handled explicitly for performance and correctness.     |
